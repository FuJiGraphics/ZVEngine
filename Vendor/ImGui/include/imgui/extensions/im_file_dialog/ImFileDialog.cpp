...
#define GUI_ELEMENT_SIZE ((GImGui->FontSize + 10.f > 24.f) ? GImGui->FontSize + 10.f : 24.f)
...
namespace ifd {
    ...
    bool FileDialog::Save(int type, const std::string& key, const std::string& title, const std::string& filter, const std::string& startingDir)
    {
	...
	void FileDialog::m_renderFileDialog()
	{
		...
		if (ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && escapeKey >= 0 && ImGui::IsKeyPressed((ImGuiKey)escapeKey))
			m_isOpen = false;
	}
}
