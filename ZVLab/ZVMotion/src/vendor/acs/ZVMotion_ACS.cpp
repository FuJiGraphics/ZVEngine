...
namespace ZVLab {
	...
	bool CZVMotion_ACS::Initialize()
	{
		m_iMaxUseAxis = 0;
		m_bInitialized = true;
		this->BoardInit();

		FZLOG_INFO("CZVMotion_ACS: Initialization completed");
		return (true);
	}

	bool CZVMotion_ACS::Shutdown()
	{
		if (m_bInitialized)
		{
			m_bInitialized = false;
	...
	bool CZVMotion_ACS::CheckMotIndex(int iMotIndex)
	{
		if (iMotIndex < 0 || m_iMaxUseAxis <= iMotIndex) 
			return (false);
		return (true);
	}

	bool CZVMotion_ACS::BoardInit()
	{
		m_tMotComConfig.handle = (HANDLE)-1;
		FZLOG_INFO("CZVMotion_ACS: ");

		char* pchAddress;
#ifdef USE_ACS_SYMUL
		pchAddress = "127.0.0.1";
#else
		pchAddress = "10.0.0.50";
#endif
		...
		{
			FZLOG_INFO("CZVMotion_ACS BoardInit() Fail");
			return (false);
		}
		...
		FZLOG_INFO("CZVMotion_ACS BoardInit() Done");
		return (true);
	}
	...
	bool CZVMotion_ACS::MotInit(int iMotIndex, const TzvAxisInfo& tAxisInfo)
	{
		this->CheckMotIndex(iMotIndex);
		...
		if (dOnePulseDist < 0) return (false);
